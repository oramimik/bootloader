...
struct vmm_context
{
	...
	UINT64(EFIAPI *set_gdt_idt)(struct vmm_context *context, UINT64 free_page);
	UINT64(EFIAPI *set_paging)(struct vmm_context *context, UINT64 free_page);

	UINT64(EFIAPI *start_aps)(struct vmm_context *context, UINT64 free_page);

	EFI_FILE_PROTOCOL *(EFIAPI *open_vmm)(EFI_HANDLE image_handle);
	UINTN(EFIAPI *vmm_size)(EFI_FILE_PROTOCOL *vmm_img);
	EFI_STATUS(EFIAPI *read_vmm)(struct vmm_context *context, EFI_FILE_PROTOCOL *vmm_img);
};
#pragma pack(pop)

void EFIAPI init_services(EFI_HANDLE image_handle, EFI_SYSTEM_TABLE *system_table);
