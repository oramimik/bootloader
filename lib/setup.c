...
struct vmm_context *create_vmm_context()
{
	...
	context->start = start_settings;
	context->set_gdt_idt = set_gdt_idt_register;
	context->set_paging = set_page_table;
...
void EFIAPI init_vmm_context(struct vmm_context *context)
{
	...
	status = gBS->AllocatePages(AllocateAnyPages, EfiRuntimeServicesCode, 2048, &context->vmm);
	if (EFI_ERROR(status))
	{
	...
	status = gBS->AllocatePages(AllocateMaxAddress, EfiRuntimeServicesData, 1, &context->ap_entry_page);
	if (EFI_ERROR(status))
	{
...
EFI_FILE_PROTOCOL *open_vmm_binary(EFI_HANDLE image_handle)
{
	...
	if (EFI_ERROR(status))
	{
		Print(L"Failed to HandleProtocol for Simple File System = %r\r\n", status);
		return NULL;
	}

	status = gBS->HandleProtocol(loaded_img->DeviceHandle, &gEfiSimpleFileSystemProtocolGuid, (void **)&simple);
	if (EFI_ERROR(status))
	{
	...
	status = root->Open(root, &vmm_bin, L"ssr.bin", EFI_FILE_MODE_READ | EFI_FILE_MODE_WRITE, 0);
	if (EFI_ERROR(status))
	{
...
UINTN EFIAPI get_vmm_size(EFI_FILE_PROTOCOL *vmm_img)
{
	...
	vmm_img->GetInfo(vmm_img, &gEfiFileInfoGuid, &img_size, NULL);
...
void EFIAPI start_settings(struct vmm_context *context, UINTN vmm_binary_size)
{
	...
	free_page = ((context->vmm + vmm_binary_size) + PAGE_SIZE - 1) & PAGE_MASK;

	context->vmm_stack = (EFI_PHYSICAL_ADDRESS)free_page;
	free_page += (8 * PAGE_SIZE);

	free_page = context->set_gdt_idt(context, free_page);

	free_page = context->set_paging(context, free_page);
...
UINT64 EFIAPI set_page_table(struct vmm_context *context, UINT64 free_page)
{
	...
	free_page += PAGE_SIZE;
	context->pde[1] = (UINT64)free_page;
	free_page += PAGE_SIZE;
	context->pde[2] = (UINT64)free_page;
	free_page += PAGE_SIZE;
	context->pde[3] = (UINT64)free_page;

	// I should to be identity mapping vmm start + 8MB with page table and gdt settings
	...
	{
		context->pde[i] &= PDE_FALGS_MASK;
		context->pde[i] |= ((UINT64)context->vmm + (i * 0x200000)) & PHY_ADDRESS_MASK;
	}
	...
	for (UINT64 i = 0; i < 4; ++i)
	{
		...
		Print(L"PDE[%llu] = 0x%lx\r\n", i, context->pde[i] & PHY_ADDRESS_MASK);
	}
	Print(L"\r\n\r\n\r\n");
...
{
	UINT64 gdt_addr, gdtr_addr, idt_addr, idtr_addr;
	...
	context->gdtr->offset = context->gdt[7];
	context->gdtr->limit = 0xb;

	Print(L"gdtr->offset = 0x%lx\r\n", context->gdtr->offset);
	Print(L"gdtr->limit = 0x%lx\r\n", context->gdtr->limit);

	idt_addr = gdtr_addr + 16; // current free address
	Print(L"idt addr = 0x%lx\r\n", idt_addr);

	// context->idt[0] = start setup idt now!
	...
	// next step is setup for idt

	/* idt setup */
	context->idtr[0].offset = (UINT64)&context->idt[0];
	context->idtr[0].limit = 0xff;
...
UINT64 EFIAPI start_ap_wake_up(struct vmm_context *context, UINT64 free_page)
{
	...
	mp->GetNumberOfProcessors(mp, &context->processor_num, &context->enabled_processor_num);
	...
	for (UINT64 i = 0; i < num; ++i)
	{
		...
		context->processor[i].package = (UINT64)processor_info.Location.Package;
		context->processor[i].core_addr = (UINT64)processor_info.Location.Core;
		context->processor[i].thread_addr = (UINT64)processor_info.Location.Thread;
	}
